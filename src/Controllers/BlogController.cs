namespace Miniblog.Core.Controllers
{
    ...
    public class BlogController : Controller
    {
        ...
        {
            var post = await this._blogService.GetPostById(postId).ConfigureAwait(true);
            ...
            if (post is null || !post.AreCommentsOpen(this._blogSettings.Value.CommentsCloseAfterDays))
            {
        ...
        public async Task<IActionResult> Category(string category, int page = 0)
        {
            ...
            var posts = this._blogService.GetPostsByCategory(category);
            ...
            // set the view option
            this.ViewData["ViewOption"] = this._blogSettings.Value.ListView;

            this.ViewData[Constants.TotalPostCount] = await posts.CountAsync().ConfigureAwait(true);
            this.ViewData[Constants.Title] = $"{this._webManifest.Name} {category}";
        ...
        public async Task<IActionResult> DeleteComment(string postId, string commentId)
        {
            ...
            var comment = post.Comments
                .FirstOrDefault(c => c.ID.Equals(commentId, StringComparison.OrdinalIgnoreCase));
        ...
        {
            var categories = await this._blogService
                .GetCategories()
                .ToListAsync();
        ...
        private async Task SaveFilesToDisk(Post post)
        {
            ...
            foreach (Match? match in imgRegex.Matches(post.Content))
            {
                ...
                if (base64Match.Success)
                {
                    ...
                    srcNode.Value = await this._blogService.SaveFile(bytes, fileNameNode.Value).ConfigureAwait(false);
